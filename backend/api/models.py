...
from .logic import remove_media_folder, remove_media, generate_media, get_admin_thumb, MediaFileStorage, media_upload_to
...
class Section(models.Model):
    ...
    slug = models.SlugField(
        ...
        blank=True,
        unique=True,
        help_text='Латинское название связанного с разделом элемента меню'
    )
    ...
    show_on_page = models.CharField(
        ...
        choices=PAGE,
        default=PAGE[0][0],
        help_text='Укажите страницы отображения контента'
    )
    area = models.CharField(
        ...
        choices=AREA,
        default=AREA[1][0],
        help_text='Укажите область отображения контента на странице'
    )
...
class Category(models.Model):
    title = models.CharField(
        ...
        max_length=50,
        unique=True,
        help_text='Отобразится в области фильтра по категориям'
    )
...
class Post(models.Model):
    ...
    section = models.ForeignKey(
        ...
        related_name='posts',
        verbose_name='Раздел',
        help_text='Выберите связанный с контентом раздел'
    )
    ...
    description = RichTextUploadingField(
        'Подробное описание',
        blank=True,
        help_text='Дополнительный блок с редактором текста'
    )
    sort = models.PositiveSmallIntegerField(
        'Индекс сортировки',
        null=True,
        blank=True,
        help_text='Нумерованные записи окажутся выше других'
    )
...
class Contact(Post):
    ...
    whatsapp = models.CharField('WhatsApp', max_length=75, blank=True,
                                help_text='Укажите номер телефона цифрами wa.me/7XXXXXXXXXX')
    telegram = models.CharField('Telegram', max_length=75, blank=True,
                                help_text='Укажите имя пользователя username t.me/username')
...
class Slide(Media):
    ...
    link = models.URLField(
        ...
        null=True,
        blank=True,
        help_text='Внешняя ссылка на видео (Youtube, Vimeo и др)'
    )
...
class Seo(models.Model):
    post = models.OneToOneField(
        ...
        related_name='seo',
        verbose_name='Запись',
        help_text='Выберите соответствующую запись для для отображения мета данных на странице этого контента'
    )
    section = models.OneToOneField(
        ...
        null=True,
        blank=True,
        related_name='page_seo',
        verbose_name='Секция',
        help_text='Выберите секцию для отображения мета данных на странице с этим контентом'
    )
    title = models.CharField('Заголовок страницы', max_length=250, blank=True)
    description = models.TextField(
        'Мета описание',
        blank=True,
        help_text='Описание страницы в поисковой выдаче. Рекомендуется 70-80 символов'
    )
    keywords = models.CharField(
        ...
        max_length=255,
        blank=True,
        help_text='Укажите через запятую поисковые словосочетания, '
                  'которые присутствуют в заголовке или описании самой записи. '
                  'Рекомендуется до 20 слов и не более 3-х повторов'
    )
