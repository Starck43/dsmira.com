	...
	return (
		<Form ref={formRef} id={name} className={`mx-auto my-3`} onSubmit={submitHandler} validated={validated}>
			...
			<div className="buttons-group centered pt-3 gap-3">
				...
				{whatsapp && <Button variant="secondary" type="button" data-href={`${WHATSAPP_URL}&phone=${whatsapp.slice(-11)}`} onClick={createWhatsappLink}>Whatsapp</Button>}
				{closeHandler && <Button variant="secondary" type="button" onClick={closeHandler}>{cancelBtnCaption}</Button>}
			</div>
		</Form>
	)
}
