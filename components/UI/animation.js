...
export const BlockAnimation = ({
	                               ...
	                               inview = {margin:  "0px", threshold: 0.25},
	                               effect = "slideUp",
	                               handleClick = null
                               }) => {
	...
	return (
		<Component
			...
			ref={observe}
			id={id}
			className={`${className} animation ${inView ? "visible" : "invisible"} ${style[effect]} ${style[inView ? "visible" : "invisible"]}`}
			onClick={handleClick}
		...
		return (
			...
			>
				{arr.map((item, i) => (
					<span
						key={item + i}
						style={{animationDelay: `${options.duration * i}ms`}}
					>
            {item}
          </span>
				))}
