...
const FormResponse = ({ response, handleClose, className = "alert-container" }) => {
    if (response instanceof Promise || response === null)
        return (
            <ModalDialog className={`${className}`} closeHandler={handleClose}>
                <h3 className="subtitle">Отправка сообщения...</h3>
            </ModalDialog>
        )
    else if (!response?.error && response?.data)
        return (
            <ModalDialog className={`${className} success`} closeHandler={handleClose}>
        ...
        return (
            <ModalDialog className={`${className} error`} closeHandler={handleClose}>
