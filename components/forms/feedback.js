...
const FeedbackModal = ({isShow, closeHandler, formName = "feedback-form"}) => {
	...
	else if (response && response?.data.status !== -1 && !response?.error) return (
		<ModalDialog
			title={`Сообщение ${response.data?.status === 0 ? "успешно отправлено" : "уже зарегистрировано ранее"}!`}
			show={isShow}
	...
	else if (typeof response === "undefined") return (
		<ModalDialog
	...
	else if (response?.data && (response?.data.status === -1 || response?.error)) return (
		<ModalDialog
