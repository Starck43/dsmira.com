...
/*
 * getPages([page]) - get page content grouped by a header, body and footer blocks to update whole page
 * parameter:
 ...
 * – with a parameter : <object { nav{}, meta{}, header{}, body{}, footer{}, page_route }>
 * */
export async function getPages(page = "") {
  let url = cleanDoubleSlashes(`${process.env.API_SERVER}/${API_ENDPOINTS.pages}/${page}`);
...
export async function getPageDetail(id, page) {
  let url = cleanDoubleSlashes(`${process.env.API_SERVER}/${API_ENDPOINTS.pages}/${page}/${id}`);
  const res = await fetch(url);
  return await res.json();
}
...
export async function getSection(slug, param = "") {
  let url = cleanDoubleSlashes(
    `${process.env.API_SERVER}/${API_ENDPOINTS.sections}/${slug}/?page_type=${param}`
  );
  const res = await fetch(url);
  return await res.json();
}
...
export async function getSectionDetail(id, section) {
  let url = cleanDoubleSlashes(`${process.env.API_SERVER}/${API_ENDPOINTS.sections}/${section}/${id}`);
  const res = await fetch(url);
  return await res.json();
}
...
export async function getPost(id) {
  let url = cleanDoubleSlashes(`${process.env.API_SERVER}/${API_ENDPOINTS.posts}/${id}`);
  const res = await fetch(url);
  return await res.json();
}
...
export async function getAllPosts(fields = []) {
  ...
  let url = cleanDoubleSlashes(`${process.env.API_SERVER}/${API_ENDPOINTS.posts}?${params}/`);
  const res = await fetch(url);
  return (await res.json()) || [];
}
